...
namespace HopCompost_DataAccess.Base
{
        ...
        where TEntity : class
    {
        ...
        {
            return AsQueryable().Any(where);
        }

        public virtual int Count(Expression<Func<TEntity, bool>> where, params Expression<Func<TEntity, object>>[] includeProperties)
        {
            return AsQueryable().Count(where);
        }

        public int Count(params Expression<Func<TEntity, object>>[] includeProperties)
        {
        ...
        public ResultAndMessage TryToSaveChanges()
        {
            try
            {
                _dbContext.SaveChanges();
                return ResultAndMessage.Successful;
            }
